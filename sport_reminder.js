...
type Props = {};
export default class Sport_reminder extends Component {
    callMe() {
...
// this is the onPress handler for the two buttons together
    ...
    componentWillMount() {
        AsyncStorage.getItem('language',(err,store)=>{
            if(store){
                ...
                this.setState({
                    labelStart: this.language()['from_date'],
                    labelEnd: this.language()['to_date'],
                    label_count: this.language()['choose'],
                    label_period: this.language()['choose'],
                })
        ...
        console.log(this.language()['choose'], 'zaqwsxcderfvbgt');
    ...
    checkValidity() {
        ...
        console.log(d2 > d1)
        console.log(d1,'ddddd');
        console.log(d2,'lllll');

        console.log(period, in_day, date, name, 'isValidate?')
        if (period > -1 && in_day > -1 && date['thisDay'] && date['nextDay'] && name.length > 0) {
    ...
    selectHour() {
        ...
        Picker.init({
            ...
            onPickerConfirm: data => {
                ...
                this.setState({CuStYear: data[0]});
                this.setState({CuStMonth: data[1]});
                this.setState({labelStart: data[0] + ' ' + ':' + ' ' + data[1]})
                this.checkValidity()
    ...
    selectUntil() {
        ...
        Picker.init({
            ...
            onPickerConfirm: data => {
                ...
                this.setState({labelEnd: data[0] + ' ' + ':' + ' ' + data[1]})
                this.checkValidity()
    ...
    language() {
        ...
        let fa = {
            ...
            drug_name: 'نام فعالیت ورزشی',
            start_date_usage: 'ساعت شروع فعالیت',
            end_date_usage: 'ساعت پایان فعالیت',
            usage_in_day: 'تعداد دفعات فعالیت در هفته',
            period_usage: 'فاصله بین دو فعالیت',
            minutes_before: 'از چند دقیقه قبل یادآوری شود؟',
            save: 'ذخیره',
            min_15: '۱۵ دقیقه قبل',
            min_30: '۳۰ دقیقه قبل',
            ...
            period_: ['هر روز', 'یک روز در میان', 'دو روز در میان', 'سه روز در میان', 'چها روز درمیان'],
        ...
        let en = {
            ...
            drug_name: 'Sports name',
            start_date_usage: 'Start time',
            end_date_usage: 'End time of activity',
            usage_in_day: 'Number of times per week',
            period_usage: 'Interval between two activities',
            minutes_before: 'Remember from a few minutes ago',
            save: 'Save',
            min_15: '15 minutes ago',
            min_30: '30 minutes ago',
            ...
            period_: ['Every day', 'Every other day', 'Two days in between', 'Three days in between', 'Four days in between'],
    ...
    calculate() {
        ...
        let fromDate = this.state.CuStYear + ':' + this.state.CuStMonth;
        let toDate = this.state.CuEnYear + ':' + this.state.CuEnMonth;

        let dd = {thisDay: fromDate,s:this.state.CuStYear,e:this.state.CuEnYear, nextDay: toDate};
    ...
    save_reminder() {
        ...
        Promise.resolve(RNCalendarEvents.saveEvent(this.state.drug_name,
            {
                startDate: rrr+ 'T6:00.000Z',
                endDate: rrr + 'T7:00.000Z',
                calendarId: this.state.calendarId,
                recurrenceRule: {
                    duration: null,
                    frequency: 'daily',
                    occurrence: parseInt(this.language()['in_day'].indexOf(this.state.label_period[0])) + 1,
                    interval: parseInt(this.language()['period_'].indexOf(this.state.label_period[0])) + 1,
                },
                alarms: [{
                    date: -1 * parseInt(this.state.minutes_before)
                }]
            })).then(JSON.parse).then((response) => {
            let id = 'sport_' + response;
            let event_details = {
                id: response,
                start: this.calculate()['thisDay'],
                end: this.calculate()['nextDay'],
                title: this.state.drug_name
            }
    ...
    deleteEvent(id) {
        ...
        AsyncStorage.removeItem('sport_' + id);
        ...
        return (
            <KeyboardAvoidingView style={styles.container}>
                <SegmentControl
                    ...
                    height={30}
                    onTabPress={(index) => {
                        this.setState({index: index}), console.log(this.state.index, 'ljfhfhfjks')
                    }}
                    borderRadius={5}
                ...
                <ScrollView>
                    <View style={this.state.index === 0 ? styles.container : styles.noView}>
                        ...
                        <View style={styles.tah_taghari}>
                            <TouchableHighlight
                                style={{borderRadius: 7, borderWidth: 2, margin: 3, borderColor: '#1c375c'}}
                                underlayColor={'white'}
                    ...
                    <View style={this.state.index === 1 ? styles.container : styles.noView}>
                                    ...
                                    <View style={{
                                        ...
                                        <View style={{
                                            ...
                                            <View style={{
                                                ...
                                                <Text
                                                    style={styles.titlesN}>{this.language()['start_date_usage']}</Text>
                                            </View>
                                            <View style={{
                                                width: 0.7 * width,
                                                flexDirection: 'row',
                        ...
                        />
                        <View style={this.state.events.length > 0 ? {width: 0, height: 0} : {
                            width: width,
                            height: width
                        }}>
...
const styles = StyleSheet.create({
    ...
    tah_taghari: {
        marginTop: 17, marginBottom: 7, flexDirection: 'row', justifyContent: 'center', alignItems: "center",
        borderBottomWidth: 2, borderColor: '#1c375c', paddingBottom: 7,width:0.93*width
    }
});
module.export = Sport_reminder;
