...
type Props = {};
export default class Medicine_reminder extends Component {
    callMe(){
...
// this is the onPress handler for the two buttons together
    ...
    language() {
        ...
        let fa = {
            ...
            drug_name: 'نام دارو',
            start_date_usage: 'تاریخ شروع مصرف',
            end_date_usage: 'تاریخ پایان مصرف',
            usage_in_day:'تعداد مصرف در روز',
            period_usage:'طول دوره زمانی مصرف',
            ...
            to_date:'تا تاریخ',
            in_day:[
                'روزی یکبار','روزی دوبار','روزی سه بار','روزی چهار بار','روزی پنج بار',
            ],
        ...
        let en = {
            ...
            drug_name: 'Drug name',
            start_date_usage: 'Date of first use',
            end_date_usage: 'Date of last use',
            usage_in_day:'Number of intake per day',
            period_usage:'Duration of consumption period',
    ...
    save_reminder(){
        ...
        // console.log('dlkdlfklf',RNCalendarEvents.authorizationStatus());
            ...
            console.log(i,'hi13');
            let devide= ~~(24/(oko+1));
                console.log(devide,'hi14');
                let hour = i*devide-4;
                console.log(this.calculate()['thisDay']+'T'+hour+':30:00.000Z','kazazaza');
                console.log(hour,'lzlzlndvdd');
                let event_id = 'medicine_reminder'+Math.floor(Math.random() * 101);
                console.log(event_id,'hi987');
                Promise.resolve(RNCalendarEvents.saveEvent(this.state.drug_name+'_'+this.language()['turn']+' '+i+' '+this.language()['om'],
                {
                startDate: this.calculate()['thisDay']+'T'+hour+':30:00.000Z',
                endDate: this.calculate()['thisDay']+'T'+hour+':40:00.000Z',
                calendarId: this.state.calendarId,
                recurrenceRule: {
                    duration:null,
                            frequency: 'daily',
                    interval: parseInt(this.language()['period_'].indexOf(this.state.label_period[0]))+1,
                    endDate: this.calculate()['nextDay']+'T00:00:00.000Z',
                },
                alarms: [{
                    date: -1*parseInt(this.state.minutes_before)
                }]
            })).then(JSON.parse).then((response) => {
                    let id = 'reminder_'+ response;
                    let event_details={id:response,start:this.calculate()['thisDay'],end:this.calculate()['nextDay'],title:this.state.drug_name+'_'+this.language()['turn']+' '+i+' '+this.language()['om']}
    ...
    deleteEvent(id){
        ...
        AsyncStorage.removeItem('doctor_'+id);
                    ...
                    >
                        ...
                        <SegmentControl
                            ...
                            onTabPress={(index) => {
                                switch (index) {
                                    case 0:
                                        this.setState({index_ago:index,minutes_before:15});
                                        break;
                                    case 1:
                                        this.setState({index_ago:index,minutes_before:30});
                                        break;
                                    default:
...
module.export = Medicine_reminder;
